      ...
      <div class="col-12">
         <div class="card">
            <div class="card-header">
               <h3 class="card-title">@if($department[0]->name) {{ $department[0]->name}} @else {{'N/A'}} @endif</h3>
               <div class="float-right">
                @if($user_access->add_module==1)	
                  ...
                  <a href="{{ route('department.edit', $department[0]->slug) }}"> 
                     ...
                     <div class="TableRow">
                        <div class="table_col">
                           <label class="label_content_br">Data Base Id <span class="colen">:</span></label>
                            @if($department[0]->id) {{ $department[0]->id}} @else {{'N/A'}} @endif                            
                        </div>
                        <div class="table_col">
                           <label class="label_content_br">Name <span class="colen">:</span></label>
                            @if($department[0]->name) {{ $department[0]->name}} @else {{'N/A'}} @endif                                   
                        </div>
                        
                        <div class="table_col">
                           <label class="label_content_br">Position <span class="colen">:</span></label>
                            @if($department[0]->position) {{ $department[0]->position}} @else {{'N/A'}} @endif                                      
                                    
                        </div>
                        <div class="table_col">
                           <label class="label_content_br">Added On <span class="colen">:</span></label>
                          @if(!empty($department[0]->created_at)) {{ date('d-m-Y H:i:s A', strtotime($department[0]->created_at)) }}  @else {{'N/A'}} @endif                               
                        </div>
                     </div>
                     <div class="TableRow">
                        <div class="table_col">
                           <label class="label_content_br">Added By <span class="colen">:</span></label>
                            @if($department[0]->added_by_name) {{ $department[0]->added_by_name}} @else {{'N/A'}} @endif                                      
