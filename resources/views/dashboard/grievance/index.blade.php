                            ...
                            @foreach ($data_listing as $row) @php($count++)
                              <tr>
                                 ...
                                 <td><input type="checkbox" name="sel_recds[]"
                                    id="sel_recds{{$count}}"
                                    value="{{$row->id}}" /></td>
                                 <td><a href="{{route($main_routes.'.show', ['id' => $row->id])}}">@if(!empty($row->subject)){{$row->subject}} @else {{'N/A'}} @endif </a></td>
                              <td>@if(!empty($row->grievance)){{$row->grievance}} @else {{'N/A'}} @endif</td>

                                 <td>@if(!empty($row->created_at)) {{ date('M d/Y', strtotime($row->created_at)) }}  @else {{'N/A'}} @endif</td>
                                 <td>@if(!empty($row->customer_name)){{$row->customer_name}} @else {{'N/A'}} @endif</td>
