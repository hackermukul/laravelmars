      ...
      <div class="col-12">
         <div class="card">
            <div class="card-header">
               <h3 class="card-title">@if($categories[0]->name) {{ $categories[0]->name}} @else {{'N/A'}} @endif</h3>
                     ...
                     <div class="TableRow">
                        <div class="table_col">
                           <label class="label_content_br">Data Base Id <span class="colen">:</span></label>
                            @if($categories[0]->id) {{ $categories[0]->id}} @else {{'N/A'}} @endif                            
                        </div>
                        <div class="table_col">
                           <label class="label_content_br">Name <span class="colen">:</span></label>
                            @if($categories[0]->name) {{ $categories[0]->name}} @else {{'N/A'}} @endif                                   
                        </div>
                        <div class="table_col">
                           <label class="label_content_br">Short Content <span class="colen">:</span></label>
                            @if($categories[0]->content) {{ $categories[0]->content}} @else {{'N/A'}} @endif                                      
                        </div>
                        <div class="table_col">
                           <label class="label_content_br">Position <span class="colen">:</span></label>
                            @if($categories[0]->position) {{ $categories[0]->position}} @else {{'N/A'}} @endif                                      
                                    
                        </div>
                        <div class="table_col">
                           <label class="label_content_br">Added On <span class="colen">:</span></label>
                          @if(!empty($categories[0]->created_at)) {{ date('d-m-Y H:i:s A', strtotime($categories[0]->created_at)) }}  @else {{'N/A'}} @endif                               
                        </div>
                     </div>
                     <div class="TableRow">
                        <div class="table_col">
                           <label class="label_content_br">Added By <span class="colen">:</span></label>
                            @if($categories[0]->added_by_name) {{ $categories[0]->added_by_name}} @else {{'N/A'}} @endif                                      
                                    
                        </div>
                        <div class="table_col">
                           <label class="label_content_br">Slug Url <span class="colen">:</span></label>
                            @if($categories[0]->slug) {{ $categories[0]->slug}} @else {{'N/A'}} @endif                                      
                         ...
                         <div class="table_col">
                           <label class="label_content_br">Cover Image<span class="colen">:</span></label>
                                ...
                                <span class="pip"><a target="_blank" href="{{ config('constants.options.MAINSITE').'build/assets/uploads/'.$categories[0]->image }}"><img class="imageThumb" src="{{ config('constants.options.MAINSITE').'build/assets/uploads/'.$categories[0]->image }}"  style="max-width:320px;"/></a></span>
                                @else
                                <span class="pip"><img class="imageThumb" src="{{ config('constants.options.MAINSITE').'build/assets/uploads/no-img.png' }} /></span>
                               @endif
                        </div>
                        
                        <div class="table_col">
                           <label class="label_content_br">Status <span class="colen">:</span></label>
                             @if($categories[0]->status ==1)
