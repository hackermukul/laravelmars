                    ...
                    <form action="{{ route('submitGrievance') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="related_to" id="related_to" class="form-control @error('related_to') is-invalid @enderror" required>
                                ...
                                <option value="Administration" {{ old('related_to') == 'Administration' ? 'selected' : '' }}>Administration</option>
                                <option value="Academics" {{ old('related_to') == 'Academics' ? 'selected' : '' }}>Academics</option>
                                <option value="Facilities" {{ old('related_to') == 'Facilities' ? 'selected' : '' }}>Facilities</option>
                                <option value="Others" {{ old('related_to') == 'Others' ? 'selected' : '' }}>Others</option>
                            </select>
                            @error('related_to')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
